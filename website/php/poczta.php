...
$sql = "SELECT `wiadomosci`.`id`, `wiadomosci`.`Temat`, `wiadomosci`.`Wiadomosc`, `wiadomosci`.`DataWyslania`, `wiadomosci`.`CzyOdczytane`, `uzytkownicy`.`Imie`, `uzytkownicy`.`Nazwisko` FROM `wiadomosci` INNER JOIN `uzytkownicy` ON `wiadomosci`.`idNadawcy` = `uzytkownicy`.`id` WHERE `wiadomosci`.`idAdresata` = $myId";
...
<div class="dziennikTop">Inbox</div>

<div id="ocenyTable"><table class="oceny" border="1">
  <thead>
    <tr><th class="nadawcaTh">Sender</th><th class="tematTh">Title</th><th class="dataTh">Send date</th><th class="odczytaneTh">read?</th></tr>
