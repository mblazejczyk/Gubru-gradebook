...
$sql1 = " SELECT `uzytkownicy`.`Imie`, `uzytkownicy`.`Nazwisko`, `uzytkownicy`.`id`, `uzytkownicy`.`uczoneKlasy`
          FROM `uzytkownicy` 
          INNER JOIN `klasy`
...
echo '<table class="oceny" border="1"><tr><th class="nrDziennikaFre">Class name</th><th>subject</th></tr>';
if ($result1->num_rows > 0) {
  while($row1 = $result1->fetch_assoc()) {
    echo '<tr><th>'. htmlspecialchars($row1["Imie"]).' '.htmlspecialchars($row1["Nazwisko"]).'</th><th>';
    $sql3 = "SELECT * FROM `klasy` WHERE `idSzkoly` = $idSzkoly";
    $result3 = $connect->query($sql3);

    $przedmiotyWKla = explode('&', $row1["uczoneKlasy"]);

    if ($result3->num_rows > 0) {
      while($row3 = $result3->fetch_assoc()) {
        if(in_array($row3["idKlasy"], $przedmiotyWKla)){
          echo '<label><input type="checkbox" name="'.htmlspecialchars($row1["id"]).'[]" value="'.htmlspecialchars($row3["idKlasy"]).'" checked>'.htmlspecialchars($row3["NazwaKlasy"]).'</laber>';
        }else{
          echo '<label><input type="checkbox" name="'.htmlspecialchars($row1["id"]).'[]" value="'.htmlspecialchars($row3["idKlasy"]).'">'.htmlspecialchars($row3["NazwaKlasy"]).'</laber>';
        }
