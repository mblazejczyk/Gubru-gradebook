...
{ 
  echo '<script>alert("Homework added (*demo)");</script>';
}
...
<div class="dziennikTop">New homework</div>
...
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" enctype="multipart/form-data" id="usrform">
  <label>
      Homework title: <br><input type="text" name="temat" size="64" maxlength="64" class = "inputLogin">
  </label>
  <br>
  <label>
  ...
  <br>
  <label>
      Due date: <br><input type="date" name="date" size="64" maxlength="64" class = "inputLogin">
  </label>
<br>
  For class:<br>
    ...
    while($row = $result->fetch_assoc()) {
      ...
      if ($result1->num_rows > 0) {
        while($row1 = $result1->fetch_assoc()) {
          echo '<option value="'.$row1["NazwaKlasy"].'">'.$row1["NazwaKlasy"].'</option>';
        }
      }
    }
  }
